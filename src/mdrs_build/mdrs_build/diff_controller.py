...
class DifferentialDriveController(Node):
    ...
    def publish_wheel_commands(self):
        ...
        wheel_msg.data = [self.lf, self.rf*1.1, self.lr*1.08, self.rr*1.005, self.lm, self.rm] #swap rf and rr once wiring is fixed
    ...
    def calculate_wheel_speeds(self, base_speed, angular_velocity):
        ...
        if abs(base_speed) < 0.001 and R != float('inf'):  # Pivot turn
            ...
            self.get_logger().debug(f"STARTING PIVOT TURN at speed {angular_velocity}")
            self.get_logger().debug("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
